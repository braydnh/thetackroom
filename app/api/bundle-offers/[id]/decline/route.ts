...
export async function POST(
  ...
  const { data: offer } = await admin
    .from("bundle_offers")
    .select("id, seller_id, buyer_id, status, conversation_id")
    .eq("id", id)
    .single();
  ...
  await admin.from("bundle_offers").update({ status: "declined" }).eq("id", id);

  // Notify buyer
  await admin.from("notifications").insert({
    user_id: offer.buyer_id,
    type: "bundle_declined",
    title: "Bundle offer declined",
    body: "Your bundle offer was declined by the seller.",
    link: `/messages`,
  });
